    ...
    public class OnlineController : Controller
    {
        ...
        public async Task<JsonResult> GetPaymentDepts(GetDeptsApiModel request)
        {
            ...
            using (var http = new HttpClient())
            {
                ...
                content.Headers.Add("EsParkApp-Token", Common.token);
        ...
        public JsonResult getPayment(getRequest request)
        {
            ...
            var requestId = request.VerifyEnrollmentRequestId;
        ...
        public vPayResponse vPostPayment(vPayRequest request)
        {
            ...
            var unicId = request.TransactionId;
            ...
            if (string.IsNullOrEmpty(responseFromServer))
            {
                return new vPayResponse() { Msg = "Hata", resultCode = "", resultDescription = "" };
            }
            ...
            var response = new vPayResponse() { Msg = msg, resultCode = resultCode, resultDescription = resultDescription };
            return response;
        }
        ...
        public async Task<JsonResult> setOdemeYapan(GetCustomerRequest request)
        {
            try
            {
                ...
                if (vResponse.resultCode == "0000")
                {
                    ...
                    return Json(new { status = true, msg = "Başarılı" }, JsonRequestBehavior.AllowGet);
                }
                else
                {
                    Session.RemoveAll();
                    return Json(new { status = false, msg = vResponse.Msg }, JsonRequestBehavior.AllowGet);
                }
            }
            catch (Exception ex)
            {
                Session.RemoveAll();
                return Json(new { status = false, msg = "Hata: " + ex.Message }, JsonRequestBehavior.AllowGet);
            }
        }
    }
}